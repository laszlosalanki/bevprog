...
//vedesi feladat: csomopontok megszamlalasa (beleertve a gyokeret is)
...
public:
   ...
   */
    int csomopontdb = 1; //a szamlalo 1-rol indul, mert a gyoker is csomopont
    LZWBinFa ():fa (&gyoker)
    {
    }
    ~LZWBinFa ()
    ...
    void operator<< (char b)
    {
		...
		csomopontdb++; //minden csomopont letrejottekor noveljuk a szamlalot
...
main (int argc, char *argv[])
{
    ...
    kiFile << "Csomopontok szama: " << binFa.csomopontdb << std::endl;
    kiFile.close ();
    beFile.close ();
